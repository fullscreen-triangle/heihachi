...
\title{\textbf{Searching Continuous Audio as a Whole Item:\\[3pt]
A Sequence-Individuation Algorithm for Radio, Mixes, and Sets}}
...
unresolved residual---prove its correctness and $\bigOh(\cdot)$ complexity, and
specify an evaluation protocol. The result is a music search that operates at
...
\begin{proof}
Set $\flo:=\inf_v\res(v)$. By \Cref{thm:floor-derived} each $\res(v)>0$; the
residual is the irreducible boundary content of $v$'s identification, carried by
the edges of $v$'s resting cut, so each such edge has weight at least the
per-constituent residual, hence at least $\flo$. Every contact $\{u,v\}$ is a
boundary edge of the identification of $u$ (it lies in $\cut(\{u\})$), so every
weight satisfies $\wt(e)\ge\flo$. Connectivity forces
$\cut(S^{\ast}(v))\neq\varnothing$, so $\str(v)=\wt(\cut(S^{\ast}(v)))\ge\flo>0$.
\end{proof}
...
handle, not of the position). The construction extends to any $n\ge2$ with at
least two distinct handles by permuting two of the positions. Hence a shared
content set does not fix the item.
...
\begin{proposition}[Contextual narrowing of the ambiguity set]
\label{prop:context}
For a constituent $u_i$ occupying position $i$ of an item, the ambiguity set
relevant to placing it in the signature is at most the set of constituents
consistent with its position given the already-placed neighbours---bounded, in a
sequence model that conditions on the previous $\ell$ placed handles, by the
number of pool constituents that plausibly follow that context. This is in
general far smaller than the whole pool, so a coarse handle (often the name
alone) is sufficient (\Cref{def:lsi}), and \Cref{thm:sufficiency} caps the
resolution needed.
\end{proposition}

\begin{proof}
Placement of $u_i$ in the signature requires distinguishing it only from
alternatives that could occupy position $i$ given the context, i.e.\ from
$A(u_i)=\{v:\ v\ \text{is a plausible occupant of position}\ i\}$. Any handle
separating $u_i$ from this set is sufficient (\Cref{def:lsi}); since $A(u_i)$ is
a subset of the pool determined by the local context, it is bounded by the number
of context-consistent constituents, which for a conditioning length $\ell\ge1$ is
typically a small fraction of the pool. The least sufficient identifier is then
correspondingly cheap, and by \Cref{thm:sufficiency} no finer resolution is
needed for placement.
\end{proof}

\begin{remark}[Neighbours do the work a fingerprint would]
...
\emph{in a stream} must be distinguished only from the few that fit its context
(small $A$, cheap handle). The sequence supplies, for free, most of the
individuation a standalone fingerprint would have to compute.
\end{remark}
...
\begin{theorem}[Search--match duality]
\label{thm:duality}
Analysis and synthesis are the same alignment problem read in opposite
directions. Precisely, both seek a signature $\sig$ and a target $\sig^{\ast}$
minimising $\algn(\sig,\sig^{\ast})$ (\Cref{def:score}); analysis fixes $\sig$
(the observed stream) and searches over targets $\sig^{\ast}$ in a library,
while synthesis fixes $\sig^{\ast}$ (the desired style) and searches over
constructible $\sig$ over the available pool. The objective, the alignment
operator, and the admissibility criterion are identical; only the free variable
differs.
\end{theorem}

\begin{proof}
Both queries are instances of $\min\ \algn(\sig,\sig^{\ast})$ subject to
$\algn\le\eps$ (\Cref{def:admissible}). In analysis, $\sig$ is given (the
recognised stream) and the minimisation ranges over the finite library of targets
$\{\sig^{\ast}_1,\dots\}$, returning the best-aligned target---recognition. In
synthesis, $\sig^{\ast}$ is given (the style) and the minimisation ranges over
signatures $\sig$ realisable as ordered selections from the available pool,
returning a best-aligned constructible sequence---generation. The alignment
operator $\algn$ (\Cref{def:alignment,def:score}) and the tolerance test
(\Cref{def:admissible}) are the same in both; the problems are related by
exchanging which of $\sig,\sig^{\ast}$ is bound and which is free, i.e.\ by
reading the same alignment forward (construct $\sig$ toward $\sig^{\ast}$) or
backward (recognise $\sig$ as $\sig^{\ast}$).
\end{proof}
...
\For{$i \gets 1$ \textbf{to} $n$}
  ...
  \State $A_i \gets \textsc{Context}(\hand_{i-\ell},\dots,\hand_{i-1})$
    \Comment{ambiguity set (\Cref{prop:context})}
...
the number of residual (unnamed or insufficient) constituents; acoustic cost is
paid only on the residual (\Cref{cor:name-first}), not on all $n$.
...
\begin{corollary}[The whole-item search is cheaper than per-track search]
\label{cor:cheaper}
Recognising a stream by \Cref{alg:acquire} costs
$\bigOh(n\,c_{\mathrm{sym}}+r\,c_{\mathrm{ac}})$, whereas fingerprinting every
constituent independently costs $\bigOh(n\,c_{\mathrm{ac}})$. Since
$c_{\mathrm{sym}}\ll c_{\mathrm{ac}}$ and $r\le n$ (typically $r\ll n$), the
whole-item search is cheaper by a factor approaching $c_{\mathrm{ac}}/
c_{\mathrm{sym}}$ when most constituents are nameable.
\end{corollary}

\begin{proof}
Compare $n\,c_{\mathrm{sym}}+r\,c_{\mathrm{ac}}$ against $n\,c_{\mathrm{ac}}$: the
difference is $n\,c_{\mathrm{ac}}-n\,c_{\mathrm{sym}}-r\,c_{\mathrm{ac}}=
(n-r)c_{\mathrm{ac}}-n\,c_{\mathrm{sym}}$, positive whenever
$(n-r)c_{\mathrm{ac}}>n\,c_{\mathrm{sym}}$, i.e.\ whenever the nameable fraction
$(n-r)/n$ exceeds $c_{\mathrm{sym}}/c_{\mathrm{ac}}$, which holds broadly since
$c_{\mathrm{sym}}/c_{\mathrm{ac}}$ is small. The saving grows with the nameable
fraction and the cost ratio.
\end{proof}
...
The protocol measures the realisation of the framework by concrete recognisers;
it does not test the theorems, which are proved. In particular the proved
...
small $A(v)$, hence how cheap the least sufficient identifier, actually is.

...
class CompositeSimilarity:
    ...
    def _spectral_similarity(self, source: np.ndarray, 
        ...
        for band_name, (low, high) in self.bands.items():
            ...
            source_band = librosa.effects.trim(
                librosa.filtfilt(source, self.sr, low, high)
            )[0]
            target_band = librosa.effects.trim(
                librosa.filtfilt(target, self.sr, low, high)
            )[0]
    ...
    def _compute_multiband_onsets(self, audio: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        for band_name, (low, high) in self.bands.items():
            ...
            y_band = librosa.effects.trim(
                librosa.filtfilt(audio, self.sr, low, high)
            )[0]

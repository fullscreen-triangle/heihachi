...
class AmenBreakTemplate:
    ...
    def _create_standard_pattern(self) -> np.ndarray:
        ...
        for pos in self.kick_positions:
            ...
            pattern[idx:idx+100] = self._create_drum_envelope('kick') * 1.0
            
        for pos in self.snare_positions:
            idx = int((pos/16) * pattern_length)
            pattern[idx:idx+100] = self._create_drum_envelope('snare') * 0.8
            
        for pos in self.hihat_positions:
            idx = int((pos/16) * pattern_length)
            pattern[idx:idx+100] = self._create_drum_envelope('hihat') * 0.6
    ...
    def _create_component_patterns(self) -> Dict[str, np.ndarray]:
        ...
        for pos in self.kick_positions:
            idx = int((pos/16) * pattern_length)
            kick[idx:idx+100] = self._create_drum_envelope('kick')
        ...
        for pos in self.snare_positions:
            idx = int((pos/16) * pattern_length)
            snare[idx:idx+100] = self._create_drum_envelope('snare')
        ...
        for pos in self.hihat_positions:
            idx = int((pos/16) * pattern_length)
            hihat[idx:idx+100] = self._create_drum_envelope('hihat')
